...
def compile_network(env, target, model, start_pack, stop_pack):
    ...
    mobilenetG = load_mobilenet()
    mod, params = relay.frontend.from_mxnet(mobilenetG, shape_dict)
    ...
    return relay_prog, params, mobilenetG
...
def profile():
    ...
    print("Extract tasks...")
    relay_prog, params, mobilenetG = compile_network(env, target, network, start_pack, stop_pack)
    ...
    if target.device_name != "vta":
        ...
        with vta.build_config(opt_level=2, disabled_pass={"AlterOpLayout","tir.CommonSubexprElimTIR"}):
            lib = relay.build_module.build(
                relay_prog, target=target, params=params, target_host=env.target_host
            )
    ...
    #lib.export_library("/home/harry09119/other-tvm/util/network.so")
    #lib = remote.load_module("n/home/harry09119/other-tvm/util/network.so")
    remote.upload(temp.relpath("graphlib.tar"))
    lib = remote.load_module("graphlib.tar")

    # Generate the graph executor
    ctx = remote.ext_dev(0) if device == "vta" else remote.cpu(0)
    m = debug_executor.create(lib["get_graph_json"](),lib,ctx)
    #from tvm.contrib.debugger import debug_executor as graph_runtime
    #m =graph_runtime.GraphModuleDebug(lib_["debug_create"]("default",ctx),[ctx],lib_.get_graph_json(),".")

    # upload parameters to device
    image = tvm.nd.array((np.random.uniform(size=(1, 3, 224, 224))).astype("float32"))
    m.set_input("data", image)
    m.run()
    #m_out = m.get_output(0, tvm.nd.empty((1,1000))).numpy()
    m_out = m.run_individual_node(1,repeat=2,min_repeat_ms=500,cooldown_interval_ms=1000)

    Batch = namedtuple('Batch', ['data'])
    mobilenetG.forward(Batch([mx.nd.array(image)]))
    prob = mod.get_outputs()[0].asnumpy()
    prob = np.squeeze(prob)

    mobilenetG_outs = np.argsort(prob)[::-1]

    print("TVM:",m_out)
    print("mobilenetG_out:",mobilenetG_outs[0])
#    mobilenetG_out = 
